...
import io.github.dimaniojk.nofingerprint.command.NoFingerprintCommand;
...
import io.github.dimaniojk.nofingerprint.protection.ShaderStripTracker;
...
/**
 * Client-side initialization for the NoFingerprint mod.
 * Loads configuration and initializes protection systems.
 */
public class NoFingerprintClient implements ClientModInitializer {
	@Override
	public void onInitializeClient() {
		// Log mod initialization
		NoFingerprint.LOGGER.info("{} v{} - Privacy protection for Minecraft", NoFingerprint.MOD_NAME, NoFingerprint.getVersion());
		NoFingerprint.LOGGER.info("Protecting against: TrackPack, Key Resolution Exploit, Client Fingerprinting");
		
		NoFingerprintConfig.getInstance();
		NoFingerprintCommand.register();
		...
		ClientLifecycleEvents.CLIENT_STARTED.register(client -> {
			...
			// Initial closure seed; further rebuilds run via NoFingerprintConfig.save().
		...
		NoFingerprint.LOGGER.info("NoFingerprint client protection initialized");
	}
	...
	private void scanRegisteredChannels() {
		...
		NoFingerprint.LOGGER.debug("[NoFingerprint] Scanned {} mod channels at startup", channelCount);
	}
	...
	//?}
		try {
			...
			NoFingerprint.LOGGER.debug("[NoFingerprint] Could not scan {}: {}", label, e.getMessage());
			return 0;
		}
	}
	...
	private void scanModsForLanguageFiles() {
			...
			for (Path rootPath : mod.getRootPaths()) {
				...
				if (Files.exists(langFile)) {
					...
					try (InputStreamReader reader = new InputStreamReader(Files.newInputStream(langFile))) {
						...
						if (keyCount > 0) {
							NoFingerprint.LOGGER.debug("[NoFingerprint] Fallback: Registered {} translation keys for mod '{}'", keyCount, modId);
							modsWithLang++;
							modsAdded++;
						}
					} catch (Exception e) {
						NoFingerprint.LOGGER.debug("[NoFingerprint] Could not read language file for {}: {}", modId, e.getMessage());
					}
		...
		NoFingerprint.LOGGER.debug("[NoFingerprint] Fallback scan added {} mods with translation keys", modsAdded);
		NoFingerprint.LOGGER.debug("[ModRegistry] Total: {} whitelistable mods, {} translation keys, {} keybinds",
			modsWithLang, ModRegistry.getTranslationKeyCount(), ModRegistry.getKeybindCount());
	}

	/** Record each mod's {@code assets/<ns>/shaders/} files (for {@code /nofingerprint info} + the strip's owner cache). */
				...
				try (var namespaces = Files.list(assets)) {
					namespaces.filter(Files::isDirectory).forEach(nsDir -> {
						...
						try (var files = Files.walk(shadersDir)) {
							...
							NoFingerprint.LOGGER.debug("[NoFingerprint] shader walk failed for {} ({}): {}", modId, ns, e.getMessage());
						}
					});
				} catch (Exception e) {
					NoFingerprint.LOGGER.debug("[NoFingerprint] shader scan failed for {}: {}", modId, e.getMessage());
				}
			}
		}
	}
