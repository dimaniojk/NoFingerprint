...
public class PackStripOverlay extends Screen {
    ...
    private PackStripOverlay(UUID packId, boolean required) {
        super(NoFingerprintLang.component(NoFingerprintStrings.PACKSTRIP_TITLE));
        this.packId = packId;
    ...
    protected void init() {
        ...
        y += 14;
        addCenteredStringWidget(NoFingerprintLang.component(
                required ? NoFingerprintStrings.PACKSTRIP_REQUIRED : NoFingerprintStrings.PACKSTRIP_OPTIONAL), centerX, y);
        y += 14;
        addCenteredStringWidget(NoFingerprintLang.component(NoFingerprintStrings.PACKSTRIP_STRIPPED), centerX, y);
        y += 14 + 14;

        this.addRenderableWidget(Button.builder(
                NoFingerprintLang.component(NoFingerprintStrings.PACKSTRIP_CONTINUE), b -> this.onClose())
                .bounds(centerX - buttonWidth / 2, y, buttonWidth, buttonHeight).build());

        this.addRenderableWidget(Button.builder(
                NoFingerprintLang.component(NoFingerprintStrings.PACKSTRIP_LOAD_REAL), b -> loadForReal())
    ...
    private void loadForReal() {
        ...
        try {
            ...
            NoFingerprint.LOGGER.warn("[NoFingerprint] [Load Pack For Real] reload failed: {}", e.getMessage());
        }
        this.onClose();
    }
