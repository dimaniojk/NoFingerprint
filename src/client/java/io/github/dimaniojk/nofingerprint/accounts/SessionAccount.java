package io.github.dimaniojk.nofingerprint.accounts;
...
public class SessionAccount implements Account {
    ...
    private static final String[] AZURE_CLIENT_IDS = {
            NoFingerprintConstants.AzureClientIds.MINECRAFT_PUBLIC,
            NoFingerprintConstants.AzureClientIds.XBOX_APP,
            NoFingerprintConstants.AzureClientIds.MINECRAFT_IOS,
            NoFingerprintConstants.AzureClientIds.LAUNCHER_CLIENT
    };

    private static final HttpClient HTTP_CLIENT = HttpClient.newBuilder()
            .connectTimeout(Duration.ofSeconds(NoFingerprintConstants.Retry.HTTP_TIMEOUT_SECONDS))
            .build();
    ...
    public ValidationResult fetchInfoWithRetry() {
        ...
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            ...
            if (attempt < MAX_RETRIES) {
                ...
                NoFingerprint.LOGGER.info("[NoFingerprint] Validation attempt {} failed ({}), retrying in {}ms...", 
    ...
    private ValidationResultWithRetry fetchInfoWithResultAndRetryAfter() {
        ...
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(NoFingerprintConstants.AuthUrls.PROFILE_URL))
                    .header("Authorization", "Bearer " + accessToken)
                    .timeout(Duration.ofSeconds(NoFingerprintConstants.Retry.HTTP_TIMEOUT_SECONDS))
                    .GET()
                    .build();
            ...
            if (statusCode == 429) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] Rate limited while validating token for {} (Retry-After: {}ms)", 
            ...
            if (statusCode == 401 || statusCode == 403) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] Token invalid/expired for {}: HTTP {}", 
                        username.isEmpty() ? "unknown" : username, statusCode);
                return new ValidationResultWithRetry(ValidationResult.INVALID, 0);
            }
            
            if (statusCode != 200) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] Failed to validate token: HTTP {}", statusCode);
            ...
            if (!json.has("id") || !json.has("name")) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] Invalid profile response: missing id or name");
            ...
            NoFingerprint.LOGGER.info("[NoFingerprint] Validated account: {} ({})", username, uuid);
            return new ValidationResultWithRetry(ValidationResult.VALID, 0);
            
        } catch (Exception e) {
            NoFingerprint.LOGGER.error("[NoFingerprint] Failed to validate token: {}", e.getMessage());
            return new ValidationResultWithRetry(ValidationResult.ERROR, 0);
        }
    }
    ...
    private long extractRetryAfter(HttpResponse<?> response) {
        try {
            ...
            try {
                ...
                NoFingerprint.LOGGER.debug("[NoFingerprint] Retry-After header format not recognized: {}", retryAfter);
                return 0;
            }
        } catch (Exception e) {
            return 0;
    ...
    private static final int MAX_RETRIES = NoFingerprintConstants.Retry.MAX_RETRIES;
    private static final long BASE_RETRY_DELAY_MS = NoFingerprintConstants.Retry.BASE_RETRY_DELAY_MS;
    private static final long RATE_LIMIT_BASE_DELAY_MS = NoFingerprintConstants.Retry.RATE_LIMIT_BASE_DELAY_MS;
    ...
    public boolean login() {
        if (accessToken == null || accessToken.isBlank()) {
            ...
            NoFingerprint.LOGGER.error("[NoFingerprint] Cannot login: missing token");
            return false;
        }
        ...
        if (validationResult != ValidationResult.VALID) {
            ...
            if (validationResult == ValidationResult.INVALID && hasRefreshToken()) {
                NoFingerprint.LOGGER.info("[NoFingerprint] Token invalid, attempting refresh...");
            ...
            if (validationResult != ValidationResult.VALID) {
                ...
                NoFingerprint.LOGGER.error("[NoFingerprint] Cannot login: token validation failed ({})", validationResult);
                return false;
            }
        }
        this.valid = true;
        ...
        if (uuid == null || uuid.isBlank()) {
            ...
            NoFingerprint.LOGGER.error("[NoFingerprint] Cannot login: missing UUID after validation");
            return false;
        }
        ...
        try {
            ...
            *///?}
            ...
            accessor.nofingerprint$setUser(newUser);
            ...
            accessor.nofingerprint$setUserApiService(userApiService);
            ...
            accessor.nofingerprint$setProfileKeyPairManager(profileKeyPairManager);
            ...
            //?}
            accessor.nofingerprint$setPlayerSocialManager(socialManager);
            
            NoFingerprint.LOGGER.info("[NoFingerprint] Successfully logged in as: {}", username);
            return true;
            
        } catch (Exception e) {
            this.lastError = "Login error: " + e.getMessage();
            NoFingerprint.LOGGER.error("[NoFingerprint] Failed to login: {}", e.getMessage(), e);
            return false;
        }
    }
    ...
    public boolean refreshAccessToken() {
        if (!hasRefreshToken()) {
            NoFingerprint.LOGGER.debug("[NoFingerprint] No refresh token available for {}", username);
            return false;
        }
        ...
        for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                NoFingerprint.LOGGER.info("[NoFingerprint] Attempting to refresh token for {} (attempt {})", username, attempt);
                ...
                if (msResult.token == null) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        long delay = calculateRetryDelay(attempt, 
                                ...
                                lastRetryAfterMs);
                        NoFingerprint.LOGGER.info("[NoFingerprint] MS token refresh failed, retrying in {}ms...", delay);
                        Thread.sleep(delay);
                        continue;
                    }
                ...
                if (xblResult.token == null) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        long delay = calculateRetryDelay(attempt,
                                wasRateLimited ? ValidationResult.RATE_LIMITED : ValidationResult.ERROR,
                                lastRetryAfterMs);
                        NoFingerprint.LOGGER.info("[NoFingerprint] Xbox Live auth failed, retrying in {}ms...", delay);
                        Thread.sleep(delay);
                        continue;
                    }
                ...
                if (xstsResult.result == null) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        long delay = calculateRetryDelay(attempt,
                                wasRateLimited ? ValidationResult.RATE_LIMITED : ValidationResult.ERROR,
                                lastRetryAfterMs);
                        NoFingerprint.LOGGER.info("[NoFingerprint] XSTS auth failed, retrying in {}ms...", delay);
                        Thread.sleep(delay);
                        continue;
                    }
                ...
                if (mcResult.token == null) {
                    ...
                    if (attempt < MAX_RETRIES) {
                        long delay = calculateRetryDelay(attempt,
                                wasRateLimited ? ValidationResult.RATE_LIMITED : ValidationResult.ERROR,
                                lastRetryAfterMs);
                        NoFingerprint.LOGGER.info("[NoFingerprint] Minecraft auth failed, retrying in {}ms...", delay);
                        Thread.sleep(delay);
                        continue;
                    }
                ...
                NoFingerprint.LOGGER.info("[NoFingerprint] Successfully refreshed token for {}", username);
                return true;
                ...
                if (attempt < MAX_RETRIES) {
                    ...
                    NoFingerprint.LOGGER.warn("[NoFingerprint] Refresh attempt {} failed: {}, retrying in {}ms...", 
                    ...
                    NoFingerprint.LOGGER.error("[NoFingerprint] Failed to refresh token: {}", e.getMessage());
                }
            }
        }
        return false;
    ...
    private TokenResult refreshMicrosoftTokenWithRetry() {
        ...
        for (String clientId : AZURE_CLIENT_IDS) {
            ...
            if (result.token != null) {
                NoFingerprint.LOGGER.info("[NoFingerprint] Token refresh succeeded with client ID: {}", clientId);
                return result;
            }
        ...
        NoFingerprint.LOGGER.warn("[NoFingerprint] Token refresh failed with all known client IDs");
    ...
    private TokenResult tryRefreshWithParamsAndRetry(String clientId, String scope) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(NoFingerprintConstants.AuthUrls.MS_TOKEN_URL))
            ...
            if (response.statusCode() != 200) {
                NoFingerprint.LOGGER.debug("[NoFingerprint] Refresh with client {} scope {} failed: HTTP {}", 
            ...
            NoFingerprint.LOGGER.debug("[NoFingerprint] Refresh succeeded with client {} scope {}", clientId, scope);
            return new TokenResult(json.get("access_token").getAsString(), 0, false);
            
        } catch (Exception e) {
            NoFingerprint.LOGGER.debug("[NoFingerprint] Refresh with client {} scope {} error: {}", clientId, scope, e.getMessage());
    ...
    private TokenResult tryXboxAuthWithTicketAndRetry(String rpsTicket) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(NoFingerprintConstants.AuthUrls.XBOX_AUTH_URL))
                    .header("Content-Type", "application/json")
                    .header("Accept", "application/json")
                    .POST(HttpRequest.BodyPublishers.ofString(body.toString()))
                    .timeout(Duration.ofSeconds(NoFingerprintConstants.Retry.AUTH_TIMEOUT_SECONDS))
                    .build();
            ...
            if (response.statusCode() != 200) {
                NoFingerprint.LOGGER.debug("[NoFingerprint] Xbox Live auth with ticket format failed: HTTP {} - {}", 
                        response.statusCode(), response.body().substring(0, Math.min(200, response.body().length())));
                return new TokenResult(null, retryAfterMs, wasRateLimited);
            }
            
            JsonObject json = JsonParser.parseString(response.body()).getAsJsonObject();
            NoFingerprint.LOGGER.debug("[NoFingerprint] Xbox Live auth succeeded");
            return new TokenResult(json.get("Token").getAsString(), 0, false);
            
        } catch (Exception e) {
            NoFingerprint.LOGGER.debug("[NoFingerprint] Xbox Live auth error: {}", e.getMessage());
            return new TokenResult(null, 0, false);
        }
    }
    ...
    private XSTSResult authenticateXSTSWithRetry(String xblToken) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(NoFingerprintConstants.AuthUrls.XSTS_AUTH_URL))
                    .header("Content-Type", "application/json")
                    .header("Accept", "application/json")
                    .POST(HttpRequest.BodyPublishers.ofString(body.toString()))
                    .timeout(Duration.ofSeconds(NoFingerprintConstants.Retry.AUTH_TIMEOUT_SECONDS))
            ...
            if (response.statusCode() != 200) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] XSTS auth failed: HTTP {}", response.statusCode());
            ...
            NoFingerprint.LOGGER.error("[NoFingerprint] XSTS auth error: {}", e.getMessage());
    ...
    private TokenResult authenticateMinecraftWithRetry(String xstsToken, String userHash) {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(NoFingerprintConstants.AuthUrls.MC_AUTH_URL))
                    .header("Content-Type", "application/json")
                    .header("Accept", "application/json")
                    .POST(HttpRequest.BodyPublishers.ofString(body.toString()))
                    .timeout(Duration.ofSeconds(NoFingerprintConstants.Retry.AUTH_TIMEOUT_SECONDS))
            ...
            if (response.statusCode() != 200) {
                NoFingerprint.LOGGER.warn("[NoFingerprint] Minecraft auth failed: HTTP {}", response.statusCode());
                return new TokenResult(null, retryAfterMs, wasRateLimited);
            }
            ...
            NoFingerprint.LOGGER.error("[NoFingerprint] Minecraft auth error: {}", e.getMessage());
            return new TokenResult(null, 0, false);
        }
    }
