...
public final class UpdateChecker {
    ...
    private static final String RELEASES_URL = "https://api.github.com/repos/dimaniojk/NoFingerprint/releases/latest";
    private static final String FALLBACK_RELEASE_URL = "https://github.com/dimaniojk/NoFingerprint/releases/latest";
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (tagName != null) {
                        ...
                        if (!version.equals(currentVersion)) {
                            ...
                            NoFingerprint.LOGGER.info("[NoFingerprint] Update available: {} -> {} ({})", currentVersion, version, releaseUrl);
                        } else {
                            NoFingerprint.LOGGER.debug("[NoFingerprint] Mod is up to date ({})", currentVersion);
                        }
                    }
                } else {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] GitHub API returned status {}", response.statusCode());
                }
            } catch (Exception e) {
                NoFingerprint.LOGGER.debug("[NoFingerprint] Update check failed: {}", e.getMessage());
            } finally {
                checkComplete = true;
            }
        });
    ...
    public static boolean isUpdateAvailable() {
        return checkComplete && updateAvailable && !shownThisSession
                && !NoFingerprintConfig.getInstance().getSettings().isVersionSkipped(latestVersion);
    }
    ...
    /**
     * Returns the current mod version. Delegates to {@link NoFingerprint#getVersion()}.
     */
    public static String getCurrentVersion() {
        return NoFingerprint.getVersion();
    }
}
