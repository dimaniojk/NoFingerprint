...
public final class JarIntegrityChecker {
    ...
    private static final String RELEASES_BASE_URL = "https://api.github.com/repos/dimaniojk/NoFingerprint/releases/tags/V";
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                Path jarPath = FabricLoader.getInstance()
                        .getModContainer(NoFingerprint.MOD_ID)
                        .flatMap(mod -> mod.getOrigin().getPaths().stream().findFirst())
                        .orElse(null);

                if (jarPath == null || !Files.isRegularFile(jarPath)) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] Not running from jar, skipping integrity check");
                    return;
                }

                if (!jarPath.toString().endsWith(".jar")) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] Not running from jar file, skipping integrity check");
                    return;
                }
                ...
                if (mcVersion == null) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] Could not determine Minecraft version, skipping integrity check");
                    return;
                }

                // Step 4: Fetch the GitHub release matching the current mod version
                String currentVersion = NoFingerprint.getVersion();
                HttpRequest request = HttpRequest.newBuilder()
                        .uri(URI.create(RELEASES_BASE_URL + currentVersion))
                        .header("User-Agent", "NoFingerprint-Mod/" + currentVersion)
                ...
                if (response.statusCode() == 404) {
                    ...
                    NoFingerprint.LOGGER.debug("[NoFingerprint] No release found for version {}, skipping integrity check", currentVersion);
                    return;
                }

                if (response.statusCode() != 200) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] GitHub API returned status {}, skipping integrity check", response.statusCode());
                    return;
                }
                ...
                if (!json.has("assets")) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] No assets in release response, skipping integrity check");
                    return;
                }
                ...
                if (matchingAsset == null) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] No release asset found for MC version {}, skipping integrity check", mcVersion);
                    return;
                }
                ...
                if (digestField == null || !digestField.startsWith("sha256:")) {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] No SHA-256 digest for matching asset, skipping integrity check");
                    return;
                }
                ...
                if (!MessageDigest.isEqual(expectedDigest.getBytes(), actualDigest.getBytes())) {
                    tamperDetected = true;
                    NoFingerprint.LOGGER.warn("[NoFingerprint] JAR INTEGRITY CHECK FAILED - Expected: {}, Actual: {}", expectedDigest, actualDigest);
                } else {
                    NoFingerprint.LOGGER.debug("[NoFingerprint] Jar integrity verified");
                }
            } catch (Exception e) {
                NoFingerprint.LOGGER.debug("[NoFingerprint] Integrity check failed: {}", e.getMessage());
            } finally {
                checkComplete = true;
            }
        });
    ...
    public static boolean isTamperDetected() {
        return checkComplete && tamperDetected && !shownThisSession
                && !NoFingerprintConfig.getInstance().getSettings().isTamperWarningDismissed();
    }
     ...
     * Asset names follow the pattern: nofingerprint-{range}+v{mod_version}.jar
    ...
    private static JsonObject findMatchingAsset(JsonArray assets, String mcVersion) {
        for (JsonElement element : assets) {
            ...
            if (!name.startsWith("nofingerprint-") || !name.endsWith(".jar")) continue;

            // Extract version range: between "nofingerprint-" and "+v"
            int rangeStart = 14; // "nofingerprint-".length()
