package io.github.dimaniojk.nofingerprint.command;
...
/**
 * Debug command for NoFingerprint mod.
 * Usage: /nofingerprint info [mod name]
 */
public class NoFingerprintCommand {
    
    public static void register() {
        ClientCommandRegistrationCallback.EVENT.register(NoFingerprintCommand::registerCommands);
    }
    ...
    private static void registerCommands(CommandDispatcher<FabricClientCommandSource> dispatcher, 
        ...
        dispatcher.register(
            //? if >=26.1 {
            /*ClientCommands.literal("nofingerprint")*/
            //?} else {
            ClientCommandManager.literal("nofingerprint")
            //?}
                .requires(source -> NoFingerprintConfig.getInstance().getSettings().isDebugCommand())
                .executes(NoFingerprintCommand::showHelp)
                .then(
                    //? if >=26.1 {
                    ...
                    .then(
                        ...
                        //?}
                        .suggests(NoFingerprintCommand::suggestModNames)
                ...
                .then(
                    ...
                    //?}
                    .executes(NoFingerprintCommand::showAllChannels))
        );
    }
    ...
    private static int showHelp(CommandContext<FabricClientCommandSource> ctx) {
        ...
        source.sendFeedback(header(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_HELP_HEADER)));
        source.sendFeedback(Component.empty());
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_HELP_INFO)));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_HELP_CHANNELS)));
     ...
     * {@code /nofingerprint info} is a debug command, so granularity matters; the
    ...
    private static int showOverview(CommandContext<FabricClientCommandSource> ctx) {
        ...
        source.sendFeedback(header(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_HEADER)));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_TOTAL_MODS, entries.size())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_VANILLA_KEYS, ModRegistry.getVanillaKeyCount())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_SERVER_KEYS, ModRegistry.getServerPackKeyCount())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_TOTAL_KEYS, ModRegistry.getTranslationKeyCount())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_TOTAL_KEYBINDS, ModRegistry.getKeybindCount())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_TOTAL_SHADERS, ModRegistry.getShaderCount())));
        // Known-packs total is only meaningful where the leak exists. Suppressed on hook-absent clients (same gate as /nofingerprint info).
        if (ModRegistry.isKnownPacksHookPresent()) {
            source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_TOTAL_KNOWN_PACKS, ModRegistry.getKnownPackCount())));
        }

        source.sendFeedback(Component.empty());
        source.sendFeedback(subheader(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_MODS_HEADER)));

        if (entries.isEmpty()) {
            source.sendFeedback(warning(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_NO_MODS)));
        } else {
        ...
        source.sendFeedback(Component.empty());
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_OVERVIEW_USE_INFO)));
    ...
    private static int showModInfo(CommandContext<FabricClientCommandSource> ctx, String modName) {
        ...
        if (resolved == null) {
            source.sendFeedback(error(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_NOT_FOUND, modName)));
            source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_USE_LIST)));
            return 0;
        }

        String modId = resolved.modId();
        source.sendFeedback(header(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_HEADER, resolved.displayName())));
        source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_ID, modId)));

        // Aggregate includes JIJ descendants so meta jars (e.g. fabric-api) report children's content.
        renderContentSection(source, NoFingerprintStrings.COMMAND_INFO_TRANSLATION_KEYS,
            ModRegistry.aggregateAllTranslationKeys(modId), 20);
        renderContentSection(source, NoFingerprintStrings.COMMAND_INFO_KEYBINDS,
            ModRegistry.aggregateAllKeybinds(modId), 20);
        renderContentSection(source, NoFingerprintStrings.COMMAND_INFO_CHANNELS,
            ModRegistry.aggregateAllChannelIds(modId), 20);
        renderContentSection(source, NoFingerprintStrings.COMMAND_INFO_SHADERS,
        ...
        if (ModRegistry.isKnownPacksHookPresent()) {
            ...
            source.sendFeedback(Component.empty());
            source.sendFeedback(subheader(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_KNOWN_PACKS, knownPacks.size())));
            if (knownPacks.isEmpty()) {
                source.sendFeedback(dim(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_NONE)));
            } else {
        ...
        // Only list JIJ submodules with tracked content; pure library JIJs aren't queryable via /nofingerprint info.
        List<? extends ModContainer> trackedContained = ModRegistry.getContainedMods(modId).stream()
            ...
            .toList();
        source.sendFeedback(Component.empty());
        source.sendFeedback(subheader(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_JIJ, trackedContained.size())));
        ...
        SpoofSettings.WhitelistMode whitelistMode = settings.getWhitelistMode();
        ...
        boolean directlyWhitelisted = switch (whitelistMode) {
            ...
            case CUSTOM -> settings.isModWhitelisted(modId);
            default -> false;
        };

        if (directlyWhitelisted) {
            isAllowed = true;
            statusText = NoFingerprintLang.tr(whitelistMode == SpoofSettings.WhitelistMode.AUTO
                ? NoFingerprintStrings.COMMAND_INFO_STATUS_ALLOWED_AUTO
                : NoFingerprintStrings.COMMAND_INFO_STATUS_ALLOWED_CUSTOM);
        } else if (ModRegistry.isInDependencyClosure(modId)) {
            isAllowed = true;
            statusText = NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_STATUS_ALLOWED_DEP,
                ModRegistry.resolveRequiringModName(modId));
        } else {
            isAllowed = false;
            statusText = NoFingerprintLang.tr(switch (whitelistMode) {
                case AUTO -> NoFingerprintStrings.COMMAND_INFO_STATUS_BLOCKED_AUTO;
                case CUSTOM -> NoFingerprintStrings.COMMAND_INFO_STATUS_BLOCKED_CUSTOM;
                default -> NoFingerprintStrings.COMMAND_INFO_STATUS_BLOCKED_OFF;
            });
        }
    ...
    private static int showAllChannels(CommandContext<FabricClientCommandSource> ctx) {
        ...
        source.sendFeedback(header(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_CHANNELS_HEADER)));
        ...
        if (totalChannels == 0) {
            source.sendFeedback(warning(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_CHANNELS_NONE)));
        } else {
            source.sendFeedback(Component.empty());
            source.sendFeedback(info(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_CHANNELS_TOTAL, totalChannels)));
            source.sendFeedback(dim(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_CHANNELS_LEGEND)));
        }
    ...
    /** Render a /nofingerprint info section: subheader+count, then "(none)" or up to {@code limit} items (negative = unlimited). */
    private static void renderContentSection(
        ...
        source.sendFeedback(Component.empty());
        source.sendFeedback(subheader(NoFingerprintLang.tr(headerKey, items.size())));
        if (items.isEmpty()) {
            source.sendFeedback(dim(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_NONE)));
            return;
        }
        ...
        for (String item : items) {
            if (limit >= 0 && shown >= limit) {
                source.sendFeedback(dim(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_INFO_MORE, items.size() - shown)));
                break;
            }
    ...
    private static MutableComponent header(String text) {
        return Component.literal("[NoFingerprint] ")
    ...
    private static MutableComponent modEntry(String displayName, ModRegistry.ContentCounts counts) {
        ...
        if (counts.translationKeys() > 0) {
            details.add(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_MODENTRY_KEYS, counts.translationKeys()));
        }
        if (counts.keybinds() > 0) {
            details.add(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_MODENTRY_KEYBINDS, counts.keybinds()));
        }
        if (counts.channels() > 0) {
            details.add(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_MODENTRY_CHANNELS, counts.channels()));
        }
        if (counts.knownPacks() > 0) {
            details.add(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_MODENTRY_KNOWN_PACKS, counts.knownPacks()));
        }
        if (counts.shaders() > 0) {
            details.add(NoFingerprintLang.tr(NoFingerprintStrings.COMMAND_MODENTRY_SHADERS, counts.shaders()));
        }
