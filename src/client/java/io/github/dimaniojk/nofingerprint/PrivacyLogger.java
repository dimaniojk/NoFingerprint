...
public class PrivacyLogger {
    ...
    private static final Map<String, Long> toastCooldowns = new LinkedHashMap<>(16, 0.75f, true) {
        ...
        protected boolean removeEldestEntry(Map.Entry<String, Long> eldest) {
            return size() > NoFingerprintConstants.Limits.MAX_TOAST_COOLDOWNS;
        }
    };
    ...
    public static void alert(AlertType type, String message) {
        if (!NoFingerprintConfig.getInstance().shouldShowAlerts()) return;
        sendMessage(type, message);
    }
    
    public static void toast(AlertType type, String title, String message) {
        if (!NoFingerprintConfig.getInstance().shouldShowToasts()) return;
        try {
            showToast(type, title, message);
        } catch (RuntimeException e) {
            NoFingerprint.LOGGER.error("[NoFingerprint] Exception in toast(): {}", e.getMessage());
        }
    }
    ...
    private static void showToast(AlertType type, String title, String message) {
        try {
            ...
            *///?}
            ...
            NoFingerprint.LOGGER.error("[NoFingerprint] Exception showing toast: {}", e.getMessage());
        }
    }
    ...
    public static void sendMessage(AlertType type, String message) {
        ...
        if (client.player == null) {
            NoFingerprint.LOGGER.info("[{}] {}", type.name(), message);
            return;
        }
        ...
        MutableComponent component = Component.literal("[NoFingerprint] ")
    ...
    public static void logDetection(String category, String details) {
        if (!NoFingerprintConfig.getInstance().isLogDetections()) return;
        NoFingerprint.LOGGER.info("[Detection:{}] {}", category, details);
    }
    ...
    public static void alertLocalPortScanDetected(String url, boolean blocked) {
        ...
        if (pendingPortScans.size() < NoFingerprintConstants.Limits.MAX_PENDING_PORT_SCANS) {
        pendingPortScans.add(hostPort);
        }
        
        if (!isToastOnCooldown("localpack_alert", NoFingerprintConstants.Timeouts.EXPLOIT_TOAST_COOLDOWN_MS)) {
            alert(AlertType.DANGER, NoFingerprintLang.tr(
                blocked ? NoFingerprintStrings.ALERT_PORTSCAN_BLOCKED : NoFingerprintStrings.ALERT_PORTSCAN_DETECTED,
                hostPort));
            toast(AlertType.DANGER, NoFingerprintLang.tr(NoFingerprintStrings.TOAST_PORTSCAN));
        }
    }
    ...
    public static void showPortScanSummary() {
        ...
        if (uniquePorts == 1) {
            ...
            alert(AlertType.DANGER, NoFingerprintLang.tr(NoFingerprintStrings.ALERT_PORTSCAN_SUMMARY_SINGLE, total, port));
        } else {
            ...
            for (String port : pendingPortScans) {
                ...
                if (++shown >= NoFingerprintConstants.Display.MAX_PORTS_TO_SHOW) {
                    if (uniquePorts > NoFingerprintConstants.Display.MAX_PORTS_TO_SHOW)
                        portsStr.append(NoFingerprintLang.tr(NoFingerprintStrings.ALERT_PORTSCAN_SUMMARY_MORE,
                            uniquePorts - NoFingerprintConstants.Display.MAX_PORTS_TO_SHOW));
                    break;
                }
            }
            alert(AlertType.DANGER, NoFingerprintLang.tr(NoFingerprintStrings.ALERT_PORTSCAN_SUMMARY_MULTI, total, portsStr.toString()));
        }

        NoFingerprint.LOGGER.info("[NoFingerprint] Port scan summary: detected {} requests to {} unique targets", total, uniquePorts);
    }
