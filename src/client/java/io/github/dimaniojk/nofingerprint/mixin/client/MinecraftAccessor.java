...
public interface MinecraftAccessor {
    ...
    @Mutable
    @Accessor("user")
    void nofingerprint$setUser(User user);
    
    @Mutable
    @Accessor("profileKeyPairManager")
    void nofingerprint$setProfileKeyPairManager(ProfileKeyPairManager manager);
    
    @Mutable
    @Accessor("userApiService")
    void nofingerprint$setUserApiService(UserApiService service);
    
    @Mutable
    @Accessor("playerSocialManager")
    void nofingerprint$setPlayerSocialManager(PlayerSocialManager manager);

    //? if >=1.20.5 {
    @Accessor("downloadedPackSource")
    DownloadedPackSource nofingerprint$getDownloadedPackSource();
    //?}
}
