...
// (dirt) shows through behind the buttons. Suppress the fill only when the NoFingerprint
...
public class TabNavigationBarMixin {
    ...
    private void nofingerprint$skipBlackHeaderFill(GuiGraphics gfx, int x1, int y1, int x2, int y2, int color,
            Operation<Void> original) {
        if (Minecraft.getInstance().screen instanceof NoFingerprintConfigScreen) return;
...
// stays in nofingerprint.client.mixins.json across all versions.
