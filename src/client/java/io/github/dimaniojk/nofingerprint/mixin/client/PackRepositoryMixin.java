 ...
 *
 * <p>This mixin narrows the auto-preserve check: NoFingerprint-tracked optional server
...
public abstract class PackRepositoryMixin {
    ...
    private boolean nofingerprint$allowUnselectOptionalServerPack(Pack pack, Operation<Boolean> original) {
        boolean vanilla = original.call(pack);
        if (!vanilla) return false;
        if (pack.getPackSource() != PackSource.SERVER) return true;
        if (!NoFingerprintConfig.getInstance().shouldStripPack()) return true;
