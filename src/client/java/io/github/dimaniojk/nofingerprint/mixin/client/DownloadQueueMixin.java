...
public class DownloadQueueMixin {
    ...
    @ModifyExpressionValue(
        ...
        require = 0)
    private Path nofingerprint$isolatePackPath(Path original, @Local(argsOnly = true) UUID packId) {
        if (!NoFingerprintConfig.getInstance().shouldIsolatePackCache()) {
            return original;
        }
        ...
        if (accountId == null) {
            NoFingerprint.LOGGER.warn("[NoFingerprint] Failed to isolate resource pack cache - account UUID is null");
            return original;
        }

        Path isolatedPath = cacheDir.resolve(accountId.toString()).resolve(packId.toString());
        NoFingerprint.LOGGER.debug("[NoFingerprint] Isolating resource pack cache for account {} -> {}", accountId, isolatedPath);
        return isolatedPath;
    }
}
//?} else {
