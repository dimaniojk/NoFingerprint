...
/**
 * Private string lookup for NoFingerprint UI text.
 *
 * <p>Loads JSON from {@code /assets/nofingerprint/nofingerprintlang/{locale}.json} directly via
 ...
 * matches.
 */
public final class NoFingerprintLang {
    private static final String DEFAULT_LOCALE = "en_us";
    private static final String PATH_PREFIX = "/assets/nofingerprint/nofingerprintlang/";
    ...
    private NoFingerprintLang() {}

    /**
     * Reload the current locale map based on the filename list vanilla is about
     * to load. Called from {@link io.github.dimaniojk.nofingerprint.mixin.client.ClientLanguageMixin}
    ...
    public static void reload(List<String> filenames) {
        ...
        NoFingerprint.LOGGER.debug("[NoFingerprint] NoFingerprintLang reloaded for locale '{}' ({} entries)", locale, current.size());
    }
    ...
    private static Map<String, String> loadLocale(String locale) {
        ...
        try (InputStream in = NoFingerprintLang.class.getResourceAsStream(path)) {
            ...
            NoFingerprint.LOGGER.warn("[NoFingerprint] NoFingerprintLang failed to load {}: {}", path, e.getMessage());
